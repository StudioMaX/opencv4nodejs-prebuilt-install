...
import { autoBuildFlags, buildWithCuda, isWithoutContrib, numberOfCoresAvailable, opencvVersion, parseAutoBuildFlags } from "./env";
...
function getRunBuildCmd(msbuildExe: string | undefined): () => Promise<void> {
  if (msbuildExe) {
    return async () => {
...
async function getSharedCmakeFlags() {
  let conditionalFlags = isWithoutContrib() ? [] : ["-DOPENCV_ENABLE_NONFREE=ON", `-DOPENCV_EXTRA_MODULES_PATH=${dirs.opencvContribModules}`];
...
async function getMsbuildIfWin(): Promise<
  | {
      path: string;
      version: string;
    }
...
export async function setupOpencv() {
  ...
  const cMakeFlags = isWin() ? getWinCmakeFlags(msbuild ? msbuild.version : "") : getSharedCmakeFlags();
  ...
  await getRunBuildCmd(isWin() ? (msbuild ? msbuild.path : undefined) : undefined)();
